...
public class DCB
{
  ...
  public static AudioInputStream shuffle(AudioInputStream in, int samplesOverlap, double paramA)
  {
    ...
    // process
    // randomised stutter
    BergManipulator m = new BergManipulator(inAudio, in.getFormat());
...
//    {
//      byte[][][] delayClip = m.getRangeOfAudioData(m.getSampleCount(ims), m.getSampleCount(ims + clipSizems));
//      m.sSetAudioData(delayClip, m.getSampleCount(ims + clipSizems), allChannels);
//
//      ims += ims + clipSizems;
...
//    }
    ...
    return new AudioInputStream(
        new ByteArrayInputStream(m.getAudioDataAsSequential()),
        in.getFormat(),
  ...
  public static AudioInputStream reverse(AudioInputStream in)
  {
    ...
    for(int i = (int) ((in.getFrameLength() - 1)*in.getFormat().getFrameSize()), j = 0;
        i >= 0;
        i -= in.getFormat().getFrameSize())
    {
