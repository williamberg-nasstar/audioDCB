...
public class BergManipulator
{
  ...
  public BergManipulator() {}
  public BergManipulator(byte[] audioData, AudioFormat audioFormat)
  {
  ...
  public void sSetAudioData(byte[][][] newData, int at, boolean[] channels) throws IllegalArgumentException
  {
    ...
    {
      if(!b) isAllChannels = false;
    }
    
    if(isAllChannels &&
        at >= 0 &&
        at + newData.length < audioData.length &&
        newData.length >= getSmoothingLen())
    {
      ...
      if(at == 0)
      {
        ...
        } else
        {
          ...
          nSetAudioData(
              getRange(newData, 0, newData.length - getSmoothingLen(), false),
              0,
              channels);
          // set smoothed part
          nSetAudioData(
              fade(
                  getRange(newData, newData.length - getSmoothingLen(), newData.length, false),
                  getRangeOfAudioData(at + newData.length - getSmoothingLen(), at + newData.length)),
              at + newData.length - getSmoothingLen(),
              channels);
        }
      } else if(at + newData.length == audioData.length)
      {
        ...
        } else
        {
          // set non-smoothed part
          nSetAudioData(
              getRange(newData, getSmoothingLen(), newData.length, false),
              at + getSmoothingLen(),
              channels);
          // set smoothed part
          nSetAudioData(
              fade(
                  getRangeOfAudioData(at, at + getSmoothingLen()),
                  getRange(newData, 0, getSmoothingLen(), false)),
              at,
              channels);
        }
      } else
      {
        ...
        //set non-smoothed part
        nSetAudioData(
            getRange(newData, getSmoothingLen(), newData.length - getSmoothingLen(), false),
            at + getSmoothingLen(),
            channels);
        
        fade(
            getRangeOfAudioData(at, at + getSmoothingLen()),
            getRange(newData, 0, getSmoothingLen() - 1, true));
        
        // set smoothed parts
        nSetAudioData(
            fade(
                getRangeOfAudioData(at, at + getSmoothingLen()),
                getRange(newData, 0, getSmoothingLen() - 1, false)),
            at,
            channels);
        nSetAudioData(
            fade(
                getRange(newData, newData.length - getSmoothingLen(), newData.length - 1, false),
                getRangeOfAudioData(at + newData.length - getSmoothingLen(), at + newData.length)),
            at + newData.length - getSmoothingLen(),
            channels);
      }
...
//        ", secondValue: " + intFromByteArraySample(second[i][j]) + "->" + secondValue);
        
//        System.out.println(firstValue + ":\t\t" +
//            String.format("%16s", Integer.toBinaryString(firstValue)).replace(' ', '0') +
//            "|" + String.format("%16s", Integer.toBinaryString(firstValue/100)).replace(' ', '0'));
//        System.out.println(secondValue + ":\t\t" +
//            String.format("%16s", Integer.toBinaryString(secondValue)).replace(' ', '0') +
//            "|" + String.format("%16s", Integer.toBinaryString(secondValue/100)).replace(' ', '0'));
        
//        System.out.println("Sum: " + (firstValue + secondValue));
        
//        TODO: figure out if my internal representation is actually correct
        
//        System.out.println("Times: (" + first.length + " - " + i + " - 1) / " + first.length + " = " + (double) (first.length - i - 1) / (double) first.length);
...
//        returnAudio[i][j] = byteArraySampleFromInt((firstValue + secondValue) / 2, first[0][0].length);
      }
    }
  ...
  {
    return new AudioInputStream(new ByteArrayInputStream(getAudioDataAsSequential()),
        audioFormat,
  ...
  // converts audioData into a sequential form, perhaps for file writing purposes
  public byte[] getAudioDataAsSequential()
  {
    byte[] returnData =
        new byte[audioData.length * audioData[0].length * audioData[0][0].length];
    ...
    for(int i = 0; i < audioData.length; i++)
    {
      for(int j = 0; j < audioFormat.getChannels(); j++)
      {
        ...
        {
          returnData[i * audioFormat.getFrameSize() +
            j * audioFormat.getChannels() +
            k] =
            audioData[i][j][k];
        }
      }
  ...
  {
    audioData =
        new byte[s.length / audioFormat.getFrameSize()]
